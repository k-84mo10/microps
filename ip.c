...
ip_protocol_register(uint8_t type, void (*handler)(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface))
{
    ...
    for (entry = protocols; entry; entry = entry->next) {
        if (entry->type == type) {
            errorf("already exists, exist=%s(0x%02x)", type, entry->type);
            return -1;
        }
    }
    ...
    protocols = entry;
    infof("registerd, type=%u", entry->type);
    return 0;
}
...
ip_iface_alloc(const char *unicast, const char *netmask)
{
    ...
    if (ip_addr_pton(netmask, &iface->netmask) == -1) {
        errorf("invalid netmask: %s", netmask);
        memory_free(iface);
        return NULL;
    }
...
ip_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    if (v != IP_VERSION_IPV4) {
        errorf("ip version mismatch: %u", v);
        return;
    }
    hlen = (hdr->vhl & 0x0f) << 2;
    if (len < hlen) {
        errorf("header length is too short: len=%zu, hlen=%u", len, hlen);
        return;
    }
    total = ntoh16(hdr->total);
    if (len < total) {
        errorf("total length is too short: len=%zu, total=%u", len, total);
        return;
    }
...
ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
    ...
    if (src == IP_ADDR_ANY) {
        ...
        if (!iface) {
            errorf("ip_iface_select() failure for src=%s", ip_addr_ntop(src, addr, sizeof(addr)));
            return -1;
        }
        if (dst != iface->broadcast && dst != IP_ADDR_BROADCAST &&
    ...
    if (NET_IFACE(iface)->dev->mtu < IP_HDR_SIZE_MIN + len) {
        errorf("too long, dev=%s, mut=%s < %zu", 
